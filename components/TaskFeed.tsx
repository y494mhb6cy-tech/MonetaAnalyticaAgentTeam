...
const taskTypeColors: Record<CompanyTaskType, string> = {
  ...
  OrderWriting: "text-blue-400 bg-blue-500/20",
  DisputeResolution: "text-orange-400 bg-orange-500/20",
...
const priorityColors = {
  ...
  P2: "text-orange-400 bg-orange-500/20 border-orange-500/30",
  P3: "text-slate-400 bg-slate-500/20 border-slate-500/30",
};
...
export default function TaskFeed({
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 border-t border-slate-700/50">
      ...
      <div className="flex-none px-4 py-3 border-b border-slate-700/50 bg-slate-800/50">
        <div className="flex items-center gap-3 mb-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-sm border border-slate-700 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500 bg-slate-800 text-white placeholder:text-slate-500"
            />
          </div>
          <select
            value={sortField}
            onChange={(e) => setSortField(e.target.value as SortField)}
            className="px-3 py-2 text-sm border border-slate-700 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500 bg-slate-800 text-white"
          >
      ...
      <div className="flex-1 overflow-y-auto">
        {filteredAndSortedTasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-slate-500">
            <Filter className="w-12 h-12 mb-2 text-slate-600" />
            ...
            {filteredAndSortedTasks.map((task) => {
              ...
              return (
                <button
                  ...
                  className={`w-full ${rowHeight} px-4 flex items-center gap-3 hover:bg-slate-800/50 transition-colors text-left ${
                    isSelected ? "bg-indigo-500/20 border-l-4 border-l-indigo-500" : ""
                  }`}
                >
                  {/* Type icon */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {task.revenueImpact === "Revenue" && (
                        <TrendingUp className="flex-none w-3 h-3 text-green-400" />
                      )}
                    </div>
